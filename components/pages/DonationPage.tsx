...
const DonationPage: React.FC = () => {
  ...
  if (user) {
    ...
    enteringMethod =
      changingMethod || (!user.subscriptionId && !user.paymentMethod && user.donationAmount)
        ? true
        : false;
  ...
  let fee: number | string =
    Math.round(((user?.donationAmount || 0) * stripeFeePercent + stripeFixedFee) * 100) / 100;
    ...
    <IonPage>
      ...
      <IonContent>
        <div className="flex flex-col items-center justify-center w-full min-h-full">
          <div
            className="flex flex-col items-stretch justify-center p-6 py-8 bg-white rounded-lg dark:bg-light"
            style={{ maxWidth: '460px' }}
          >
            {user?.objectId && (
              <>
                <TextDivider>{`Godible Pro ${
                  user?.subscriptionId ? 'Settings' : 'Upgrade'
                }`}</TextDivider>
                ...
                {user?.subscriptionId && (
                  <div className="w-full pt-4 font-bold text-center text-primary">
                    Active
                    <span className="font-normal text-medium">{` (next charge ${
                      user?.nextPaymentTime
                        ? new Date(user?.nextPaymentTime).toLocaleDateString()
                        : ''
                    })`}</span>
                  </div>
                )}
                {!user?.subscriptionId && hasAccess && (
                  <div className="w-full pt-4 font-medium text-center text-danger">
                    Canceled
                    <span className="font-normal text-medium">{` (access until ${
                      lastAccessTime ? new Date(lastAccessTime).toLocaleDateString() : ''
                    })`}</span>
                  </div>
                )}
                <div className="flex flex-col items-center w-full">
                  {!enteringCustomAmount && (
                    <div className="flex py-4 text-light dark:text-dark">
                      ...
                      {!user?.donationAmount || selectingAmount ? (
                        <span className="px-2 text-2xl font-medium text-center">
                          Select your subscription
                        </span>
                      ) : (
                        <div className="flex items-center">
                          <span className="pb-1 text-2xl font-medium border-b-4 border-primary">
                            monthly
                          </span>
                        </div>
                      )}
                  ...
                  {!enteringCustomAmount && selectingAmount && (
                    <div className="flex flex-wrap justify-center w-full">
                      <IonChip
                        onClick={() => {
                          handleSelectDonation(5);
                        }}
                        disabled={isLoading}
                      >
                        <span className="pr-1 text-xs text-light dark:text-dark">$</span>
                        <span className="text-lg font-bold">5</span>
                      </IonChip>
                      <IonChip
                        onClick={() => {
                          handleSelectDonation(7);
                        }}
                        disabled={isLoading}
                      >
                        <span className="pr-1 text-xs text-light dark:text-dark">$</span>
                      ...
                      <IonChip
                        onClick={() => {
                          handleSelectDonation(21);
                        }}
                        disabled={isLoading}
                      >
                        <span className="pr-1 text-xs text-light dark:text-dark">$</span>
                        <span className="text-lg font-bold tracking-tighter">21</span>
                      </IonChip>

                      <IonChip onClick={() => handleSelectDonation(40)} disabled={isLoading}>
                        <span className="pr-1 text-xs text-light dark:text-dark">$</span>
            ...
            {enteringCustomAmount && (
              <IonItem>
                <IonLabel>$</IonLabel>
                <IonInput placeholder="Enter an amount..." ref={customAmountInput}></IonInput>
                <IonButton
                  color="primary"
                  onClick={e => {
                ...
                {showingMinNote && (
                  <IonNote slot="helper">
                    <span className="text-xs text-primary">Enter between $3 to $400</span>
                  </IonNote>
                )}
              </IonItem>
            )}
            ...
            {!enteringMethod && user?.paymentMethod && (
              <div className="flex flex-col px-5 pt-3 pb-6 border rounded-md">
                ...
                <div className="flex justify-between text-sm font-medium text-medium">
                  ...
                  <span>
                    Expires {user.paymentMethod.card.exp_month}/
                    {user.paymentMethod.card.exp_year.toString().slice(-2)}
                  </span>
            ...
            {user?.paymentMethod && !user?.subscriptionId && (
              <span className="w-full py-2 italic text-center">Card added. One last step...</span>
            )}
              ...
              <div className="flex flex-col w-full mt-6">
                <IonItem>
                  ...
                  <div className="hidden xs:inline">
                    <IonLabel>
                      <span className="text-md">{`Help cover the $${fee} transaction fee?`}</span>
                    </IonLabel>
                  </div>
                  <div className="inline xs:hidden">
                    <IonLabel>
                      <span className="text-sm">{`Cover transaction fee ($${fee})?`}</span>
                    </IonLabel>
                  </div>
                </IonItem>
              </div>
              ...
              <div className="w-full py-4">
                <IonButton
                  ...
                  onClick={async () => {
                    ...
                    if (postal_code.current?.value)
                      address.postal_code = postal_code.current?.value;
                ...
                <div className="flex items-start justify-center w-full py-2 space-x-1">
                  ...
                  <div className="flex flex-col w-full">
                    <div className="font-medium text-md xs:text-lg">
                      Link an account to your subscription
                    </div>
  ...
  return (
    <IonPage>
      ...
      <IonContent className="ion-padding">
        <div className="flex justify-center w-full">
          <IonIcon icon={heartCircle} color="primary" size="large" />
        </div>

        <h2 className="w-full text-center">Thank you for your support</h2>
        <p className="w-full pb-2 text-center text-medium">
          Godible Pro access will be granted shortly pending your first payment.
        </p>
